...
public class StudentController {
    ...
    @PutMapping("/{idstudent}/rating/{idcourse}")
    public ResponseEntity<?> updateRating(@PathVariable Integer idstudent, @PathVariable Integer idcourse, @RequestBody Rating rating) {
		Student student;
    	Course course;
    	try {
    		student = studentService.getStudent(idstudent);
    	...
    	try {
    		...
    		return ResponseMessageUtil.from(ApiReturnMessage.STUDENT_RATING_UPDATED, student.getId(), rating.getValue(), course.getId());
		} catch (BusinessErrorException e) {
			return ResponseMessageUtil.from(e.getApiReturnMessage(), e.getArgs());
		}
    }
